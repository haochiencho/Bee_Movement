...
Declare_Any_Class( "Tetrahedron",              // A demo of flat vs smooth shading.  Also our first 3D, non-planar shape.
  ...
  Declare_Any_Class( "Pyramid",              // A demo of flat vs smooth shading.  Also our first 3D, non-planar shape.
    ...
    { 'populate'()
        {
          var a = 1/Math.sqrt(3);
          var rt_3_2 = Math.sqrt(3) / 2;

          this.positions.push( vec3(.5,.5,0), vec3(.5,-.5,0), vec3(0,0,rt_3_2) );         // Method 2:  A tetrahedron with four independent triangles.
          this.positions.push( vec3(.5,-.5,0), vec3(-.5,-.5,0), vec3(0,0,rt_3_2) );
          this.positions.push( vec3(-.5,-.5,0), vec3(-.5,.5,0), vec3(0,0,rt_3_2) );
          this.positions.push( vec3(-.5,.5,0), vec3(.5,.5,0), vec3(0,0,rt_3_2) );
          this.positions.push( vec3(.5,.5,0), vec3(.5,-.5,0), vec3(-.5,-.5,0) );
          this.positions.push( vec3(-.5,-.5,0), vec3(-.5,.5,0), vec3(.5,.5,0) );

          this.normals.push( vec3(rt_3_2,0,.5), vec3(rt_3_2,0,.5), vec3(rt_3_2,0,.5) );           // Here's where you can tell Method 2 is flat shaded, since
          this.normals.push( vec3(0,rt_3_2 * -1.0,.5), vec3(0,rt_3_2 * -1.0,.5), vec3(0,rt_3_2 * -1.0,.5) );           // each triangle gets a single unique normal value.
          this.normals.push( vec3(rt_3_2 * -1,0,.5), vec3(rt_3_2 * -1,0,.5), vec3(rt_3_2 * -1,0,.5) );
          this.normals.push( vec3(0,rt_3_2,.5), vec3(0,rt_3_2,.5), vec3(0,rt_3_2,.5) );
          this.normals.push( vec3(0,0,-1), vec3(0,0,-1), vec3(0,0,-1) );
          this.normals.push( vec3(0,0,-1), vec3(0,0,-1), vec3(0,0,-1) );

          this.texture_coords.push( vec3(.625,.2165,0), vec3(.5,0,0), vec3(.75,0,0) );    // Each face in Method 2 also gets its own set of texture coords
          this.texture_coords.push( vec3(.625,.2165,0), vec3(.5,0,0), vec3(.75,0,0) );    //(half the image is mapped onto each face).  We couldn't do this
          this.texture_coords.push( vec3(.625,.2165,0), vec3(.5,0,0), vec3(.75,0,0) );    // with shared vertices -- after all, it involves different results
          this.texture_coords.push( vec3(.625,.2165,0), vec3(.5,0,0), vec3(.75,0,0) );    // when approaching the same point from different directions.
          this.texture_coords.push( vec3(1,1,0), vec3(.9,1,0), vec3(1,.9,0) );
          this.texture_coords.push( vec3(.9,.9,0), vec3(.9,1,0), vec3(1,.9,0) );
